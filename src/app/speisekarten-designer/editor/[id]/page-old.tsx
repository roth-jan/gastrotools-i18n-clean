...
export default function MenuEditor() {
  ...
  const loadMenu = async () => {
    try {
      ...
      // Always ensure we have a valid menuData object
      const safeMenuData = {
      ...
      if (params.id === 'new') {
        ...
        try {
          // Load existing menu with robust error handling
          ...
          if (response.ok) {
            ...
            // Triple-safe defensive programming
            const menu = data?.menu || {}
            const content = menu?.content || menu || {}
            
            setMenuData({
              title: content?.title || menu?.name || 'Speisekarte',
              subtitle: content?.subtitle || '',
              sections: Array.isArray(content?.sections) ? content.sections : [],
              template: content?.template || menu?.template || 'elegant-classic'
            })
          } else {
            console.warn('Menu API returned:', response.status)
  ...
  const styles = getTemplateStyles(menuData?.template || 'elegant-classic')
